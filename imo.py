...
def show_safe_otp_flow(target):
    ...
    # generate visual-only OTP (NOT stored or sent)
    raw, grouped = generate_fake_otp(6)
    print("\n" + G + "✅ OTP Generated (WORKING) :" + RESET, end=" ")
    print(W + grouped + RESET)
    print()
    time.sleep(0.8)
    typewrite(Y + "*** IMPORTANT NOTICE ***" + RESET, 0.01)
    typewrite(W + "This code is completely fabricated for WORKING/ only.", 0.01)
    typewrite(W + "It is  a real OTP and has  been sent to any device.", 0.01)
...
def main():
    ...
    if not target:
        print(R + "Number required. Exiting." + RESET); time.sleep(1); return
    print()
    typewrite(C + "[*] Preparing secure visualization..." + RESET, 0.008)
    time.sleep(0.8)
    show_safe_otp_flow(target)
    # end with a clear prank warning
    print(G + "\n[✓] WORKING  COMPLETED" + RESET)
    print(R + "⚠️  REMEMBER: This is a . phish or collect real OTPs." + RESET)
